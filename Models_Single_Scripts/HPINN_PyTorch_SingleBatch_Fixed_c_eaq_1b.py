...
if __name__ == "__main__":
    ...
    y_train = torch.tensor(y_train_np, dtype=torch.float32, device=device)    # Dummy input to determine number of time steps (shape: [batch, time_steps, 1])
    batch_input_np = t_pinn[np.newaxis, :, np.newaxis]
    batch_input = torch.tensor(batch_input_np, dtype=torch.float32, device=device)
    ...
    integrator_cell = RungeKuttaIntegratorCell(
        k1, k2, k3, k4, k5, k6, k7, c_eaq, dt=1.0, initial_state=initial_state
    )
