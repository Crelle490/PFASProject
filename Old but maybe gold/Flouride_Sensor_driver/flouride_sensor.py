...
IR_CONC = 0x0003  # word pair: 0x0003, 0x0004  -> mg/L
IR_MV   = 0x0007  # word pair: 0x0007, 0x0008  -> mV
IR_I1   = 0x0009  # word pair: 0x0009, 0x000A  -> mA
IR_I2   = 0x000B  # word pair: 0x000B, 0x000C  -> mA
...
class FluorideAnalyzer:
    ...
    def read_concentration_mgL(self) -> float:
        return self._read_float_ir(IR_CONC)

    def read_electrode_mV(self) -> float:
        return self._read_float_ir(IR_MV)

    def read_current1_mA(self) -> float:
        return self._read_float_ir(IR_I1)

    def read_current2_mA(self) -> float:
        return self._read_float_ir(IR_I2)
