...
for dt in DT_LIST:
    ...
    # Inspect the cell (optional debug)
    rnn_layer = next((ly for ly in model.layers if isinstance(ly, tf.keras.layers.RNN)), None)
    if rnn_layer is not None:
        cell = rnn_layer.cell
        print("Cell class:", type(cell))
        print("Cell.initial_state (build-time dummy):", getattr(cell, "initial_state", None))
